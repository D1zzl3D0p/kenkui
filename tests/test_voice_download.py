"""Tests for src/kenkui/voices/download.py"""
import pytest
...
def test_voices_are_present_false_when_dir_missing(tmp_path):
    from kenkui.voices.download import voices_are_present, _VOICES_LOCAL_DIR
    with patch("kenkui.voices.download._VOICES_LOCAL_DIR", tmp_path / "nonexistent"):
        assert voices_are_present() is False


def test_voices_are_present_false_when_empty(tmp_path):
    from kenkui.voices.download import voices_are_present
    compiled = tmp_path / "compiled"
    compiled.mkdir()
    with patch("kenkui.voices.download._VOICES_LOCAL_DIR", tmp_path):
        assert voices_are_present() is False
...
def test_download_voices_calls_snapshot_download(tmp_path):
    ...
    with (
        ...
        patch("kenkui.voices.download.get_registry") as mock_reg,
    ):
