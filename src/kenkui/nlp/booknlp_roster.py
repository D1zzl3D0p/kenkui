...
from ._filters import _is_proper_name  # noqa: F401 (re-exported for tests)
...
def build_roster_from_booknlp(
    ...
    for char_data in data.get("characters", []):
        ...
        aliases = [entry["n"] for entry in proper_entries if entry.get("n") and _is_proper_name(entry["n"])]
        if not aliases:
            continue
